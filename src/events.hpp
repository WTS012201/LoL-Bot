...
public:
  ...
  operator+=(const handler_type &handler) {
    return add(handler);
  }
  ...
  std::future<void> call_async(Args... params) const {
    return std::async(
        std::launch::async,
        [this](Args... asyncParams) { call(asyncParams...); }, params...);
  }
  ...
  inline typename handler_type::handler_id_type
  add(const typename handler_type::handler_func_type &handler) {
...
private:
  ...
  void call_impl(const handler_collection_type &handlers,
                 Args... params) const {
    // call all Foo(T a, ..., T z) in event
...
public:
  ev::event<unsigned int> interruptSignal;
  ev::event<> job;
  ev::event<> watch;
